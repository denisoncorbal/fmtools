...
public class RoleValidator implements ConstraintValidator<RoleValidatorAnnotation, String> {

    private List<String> validValues = new ArrayList<>();

    @Override
    public void initialize(RoleValidatorAnnotation constraintAnnotation) {
        // Optional: Initialize validator with annotation parameters
        this.validValues = Arrays.stream(Role.values()).map(fp -> fp.name()).toList();
    }
